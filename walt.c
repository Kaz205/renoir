...
#define WALT88b9b0b9ce (0x456+6420-0x1d6a)
#define WALT19b86f3ee0 (0xa96+1729-0x1156)
#define WALT53b6c10420 9500000
#define MAX_NUM_CGROUP_COLOC_ID (0x12f3+270-0x13ed)
#define WINDOW_STATS_RECENT		(0x555+2391-0xeac)
#define WINDOW_STATS_MAX		(0xe51+1595-0x148b)
#define WINDOW_STATS_MAX_RECENT_AVG	(0x1a1d+5-0x1a20)
#define WINDOW_STATS_AVG		(0x133+1902-0x89e)
#define WINDOW_STATS_INVALID_POLICY	(0x1146+57-0x117b)
#define MAX_NR_CLUSTERS			(0xaad+3213-0x1737)
#define FREQ_REPORT_MAX_CPU_LOAD_TOP_TASK	(0x483+279-0x59a)
#define FREQ_REPORT_CPU_LOAD			(0x181b+3534-0x25e8)
#define FREQ_REPORT_TOP_TASK			(0xd17+2233-0x15ce)
...
WALT482beda45b=ktime_get();WALT42c0a8ecd1=true;return(0x9e2+2544-0x13d2);}static
...
WALT3c69fe9894);return(0x1f7+4018-0x11a9);}late_initcall(WALT5a716155ff);static 
void WALT04b55e82dd(const cpumask_t*cpus,unsigned long*flags){int cpu;int 
WALT8025f196c5=(0xab6+4079-0x1aa5);local_irq_save(*flags);for_each_cpu(cpu,cpus)
{if(WALT8025f196c5==(0x1225+3990-0x21bb))raw_spin_lock(&cpu_rq(cpu)->lock);else 
...
(0x157c+346-0x16d6)...MAX_MARGIN_LEVELS-(0xe8+5114-0x14e1)]=(0x1d42+1816-0x2024)
};unsigned int sysctl_sched_capacity_margin_down[MAX_MARGIN_LEVELS]={[
(0x436+1159-0x8bd)...MAX_MARGIN_LEVELS-(0x164a+325-0x178e)]=(0x1574+3704-0x1f37)
...
(0x2e5+7891-0x2154);__read_mostly unsigned int WALTa591b9e54c=
(0x153c+268-0x1643);static __read_mostly unsigned int WALTdedb054041=
(0x262+7152-0x1e51);__read_mostly unsigned int sysctl_sched_window_stats_policy=
...
(0x198d+2310-0x2199));__read_mostly unsigned int sched_ravg_window=
...
(0x119a+2334-0x1aa9);unsigned int max_possible_capacity=(0x222d+1886-0x258b);
unsigned int min_max_possible_capacity=(0x13c4+1482-0x158e);__read_mostly 
...
(0xaea+1832-0x1211))*sizeof(unsigned long);__read_mostly unsigned int 
...
(0x246d+47-0x249b));return-EINVAL;}sched_ravg_window=WALT4ae816ebd1;return
(0xe4d+2630-0x1893);}early_param(
...
WALT23b9d3f649;return(0x1ea1+311-0x1fd8);}early_param(
...
WALTb49c1cb1d4[NR_CPUS*(0x94d+5769-0x1fc6)];int WALT5d971be8b8,WALT2dc758aa44=
(0x5b9+7133-0x2196);int WALT8c35dbc183=NR_CPUS*(0x27f+1935-0x9fe);WALT33b027c8ca
...
for(WALT5d971be8b8=(0x1570+3761-0x2421);WALT5d971be8b8<nr_cpu_ids;WALT5d971be8b8
...
WALTb49c1cb1d4);for(WALT5d971be8b8=(0x19fa+501-0x1bef),WALT2dc758aa44=
(0x1af2+2289-0x23e3);WALT5d971be8b8<nr_cpu_ids;WALT5d971be8b8++)WALT2dc758aa44+=
...
grp_time.nt_prev_runnable_sum);for(WALT5d971be8b8=(0xf92+5413-0x24b7);
...
({							\
	if (unlikely(!!(WALT8f713ea8e0)) && !WALT22b60684f8) {	\
		WALT22b60684f8 = (0xff5+4255-0x2093);			\
		WALT71e92e3d78();				\
...
window_start;if(WALT17abc14e87<(0x7ad+2974-0x134b)){printk_deferred(
...
,rq->cpu,WALT790680e4aa,rq->wrq.window_start);WALT609fba09ac((0xc24+5060-0x1fe7)
...
struct task_struct*WALT0609dbaf9d;int WALTd7f6dc4db2=(0x686+1202-0xb2e);rq->wrq.
ed_task=NULL;if(!WALTe99faa0ca8()||!rq->cfs.h_nr_running)return(0x221+36-0x245);
list_for_each_entry(WALT0609dbaf9d,&rq->cfs_tasks,se.group_node){if(!
WALTd7f6dc4db2)break;if(WALT30dc154c3b(WALT0609dbaf9d,WALT790680e4aa)){rq->wrq.
ed_task=WALT0609dbaf9d;return(0x35d+6243-0x1bbf);}WALTd7f6dc4db2--;}return
(0x4bf+3572-0x12b3);}void walt_sched_account_irqstart(int cpu,struct task_struct
*curr){struct rq*rq=cpu_rq(cpu);if(!rq->wrq.window_start)return;raw_spin_lock(&
rq->lock);WALT1360b17c9b(curr,cpu,sched_ktime_clock());raw_spin_unlock(&rq->lock
);}void walt_sched_account_irqend(int cpu,struct task_struct*curr,u64 
WALT17abc14e87){struct rq*rq=cpu_rq(cpu);unsigned long flags;
raw_spin_lock_irqsave(&rq->lock,flags);walt_update_task_ravg(curr,rq,IRQ_UPDATE,
sched_ktime_clock(),WALT17abc14e87);raw_spin_unlock_irqrestore(&rq->lock,flags);
...
push_task=NULL;}rq->active_balance=(0x76b+6558-0x2109);
...
prev_top;u8 prev=(0xaff+4972-0x1e6a)-rq->wrq.curr_table;if(!WALT6b2e94bfe7){int 
WALT1d278c4a11=NUM_LOAD_INDICES-(0x636+2740-0x10e9);if(!test_bit(WALT1d278c4a11,
rq->wrq.top_tasks_bitmap[prev]))return(0x1b92+2352-0x24c2);else return 
sched_load_granule;}else if(WALT6b2e94bfe7==NUM_LOAD_INDICES-(0x1d4+4684-0x141f)
){return sched_ravg_window;}else{return(WALT6b2e94bfe7+(0x47d+3174-0x10e2))*
...
WALT514f31b9a6,WALT3d3b61fd9a=(0xc9c+3854-0x1baa);struct task_struct*
...
(0x842+1022-0xbdc));}WALT02a8afcd23:trace_sched_load_to_gov(rq,aggr_grp_load,
...
(0x1141+4373-0x21f2)+WALTf4f061ddc1),walt_cpu_util_freq_divisor);if(
...
rq->wrq.old_estimated_time=pl;nl=div64_u64(nl*((0x186a+1933-0x1f93)+
...
struct walt_cpu_load WALTd64f78c90a={(0x91b+6985-0x2464)};unsigned long 
WALTb0b0149580=(0x15a+7544-0x1ed2),WALT53081359ce=(0x6c1+4771-0x1964);unsigned 
...
WALT568475e381=(0xa67+1411-0xf86);WALTb0b0149580=WALT1c84aa4dd0(WALTb0b0149580,
...
(WALT5d971be8b8=(0xd4b+3070-0x1949);WALT5d971be8b8<NUM_TRACKED_WINDOWS;
...
WALT5d971be8b8].subs=(0x1519+463-0x16e8);WALTfd921988bb[WALT5d971be8b8].new_subs
=(0xeeb+2059-0x16f6);}WALT609fba09ac((s64)rq->wrq.prev_runnable_sum<
(0xb6a+3226-0x1804));WALT609fba09ac((s64)rq->wrq.curr_runnable_sum<
(0x841+7715-0x2664));WALT609fba09ac((s64)rq->wrq.nt_prev_runnable_sum<
(0x571+8144-0x2541));WALT609fba09ac((s64)rq->wrq.nt_curr_runnable_sum<
(0x678+3029-0x124d));}static inline void WALT06281e0b6d(struct rq*rq,u64 ws,int 
WALT6b2e94bfe7){rq->wrq.load_subs[WALT6b2e94bfe7].window_start=ws;rq->wrq.
load_subs[WALT6b2e94bfe7].subs=(0x55d+5637-0x1b62);rq->wrq.load_subs[
WALT6b2e94bfe7].new_subs=(0x6d3+3482-0x146d);}static int get_top_index(unsigned 
...
==NUM_LOAD_INDICES)return(0x81c+1748-0xef0);return NUM_LOAD_INDICES-
(0x3f7+1316-0x91a)-WALT6b2e94bfe7;}static bool WALT433ae47377(struct rq*rq,u64 
ws){int WALT5d971be8b8;u64 WALT52cf8663bf=ULLONG_MAX;int WALTf9248c507e=
(0xc57+1133-0x10c4);for(WALT5d971be8b8=(0x14aa+3908-0x23ee);WALT5d971be8b8<
...
WALT5d971be8b8]=(0x425+5896-0x1b2d);}if(WALT0609dbaf9d->wts.prev_window_cpu[
...
(0x24b5+383-0x2634);}}raw_spin_unlock(&cluster->load_lock);}static inline void 
...
(WALT0609dbaf9d);WALT609fba09ac((0x91c+1103-0xd6a));}WALTd01f3eedf0->wrq.
...
(WALT0609dbaf9d);WALT609fba09ac((0x5c9+7459-0x22eb));}WALTd01f3eedf0->wrq.
...
WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac((0xd5a+3646-0x1b97));}
...
WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac((0x655+1750-0xd2a));}
WALTd01f3eedf0->wrq.nt_prev_runnable_sum-=WALT0609dbaf9d->wts.prev_window_cpu[
task_cpu];}WALT0609dbaf9d->wts.curr_window_cpu[task_cpu]=(0xcb+2131-0x91e);
WALT0609dbaf9d->wts.prev_window_cpu[task_cpu]=(0x397+5279-0x1836);WALT0dd808419c
...
(0xd3c+4707-0x1f9e)));}static void WALT9be28a1f9c(struct task_struct*
...
WALT146a962968[WALT6b2e94bfe7]-=(0x1356+3400-0x209d);WALTbf29d6b684[
WALT6b2e94bfe7]+=(0x1737+3744-0x25d6);if(!WALT146a962968[WALT6b2e94bfe7])
__clear_bit(NUM_LOAD_INDICES-WALT6b2e94bfe7-(0x839+7585-0x25d9),WALTd01f3eedf0->
wrq.top_tasks_bitmap[WALT792740f2d5]);if(WALTbf29d6b684[WALT6b2e94bfe7]==
(0x10df+3488-0x1e7e))__set_bit(NUM_LOAD_INDICES-WALT6b2e94bfe7-
(0x1053+910-0x13e0),WALT38ee64137f->wrq.top_tasks_bitmap[WALT6f47937b7c]);if(
...
WALT722eaaf4cf);}if(prev_window){WALT792740f2d5=(0x42b+7755-0x2275)-
WALT792740f2d5;WALT6f47937b7c=(0x13e4+2199-0x1c7a)-WALT6f47937b7c;WALT146a962968
...
WALT146a962968[WALT6b2e94bfe7]-=(0xa3c+3748-0x18df);WALTbf29d6b684[
WALT6b2e94bfe7]+=(0x927+4870-0x1c2c);if(!WALT146a962968[WALT6b2e94bfe7])
__clear_bit(NUM_LOAD_INDICES-WALT6b2e94bfe7-(0x172a+4046-0x26f7),WALTd01f3eedf0
->wrq.top_tasks_bitmap[WALT792740f2d5]);if(WALTbf29d6b684[WALT6b2e94bfe7]==
(0x2ea+2747-0xda4))__set_bit(NUM_LOAD_INDICES-WALT6b2e94bfe7-(0x68c+2073-0xea4),
...
TASK_UPDATE,WALT790680e4aa,(0x590+6177-0x1db1));walt_update_task_ravg(
WALTeb0037895f->curr,WALTeb0037895f,TASK_UPDATE,WALT790680e4aa,
(0x580+6470-0x1ec6));walt_update_task_ravg(WALT0609dbaf9d,task_rq(WALT0609dbaf9d
),TASK_MIGRATE,WALT790680e4aa,(0x6f4+7036-0x2270));WALT1360b17c9b(WALT0609dbaf9d
...
WALTe66a622f51){rq->wrq.window_start=(0xbcf+4155-0x1c09);WALTe66a622f51=
(0xcb4+1256-0x119b);atomic64_set(&WALT8d508f7c3b,rq->wrq.window_start);
...
(0x935+6604-0x2301);rq->wrq.nt_curr_runnable_sum=rq->wrq.nt_prev_runnable_sum=
(0x1adb+43-0x1b06);raw_spin_unlock(&WALT3e3d21be47->lock);}rq->curr->wts.
...
#define WALT07c7d3e0b1 (0x428+3195-0x109b)
#define WALT07f1c18f03 (0x3f4+6719-0x1e31)
#define WALTedc4e7b72b (0x7a9+7484-0x24d5)
#define WALTd412d902d5 (0x5b6+4010-0x1550)
static inline void WALTcd30d5f093(u8*WALTc0f271aeff,int WALT8aa1674e1f){int 
WALT5d971be8b8,WALT2a39967a03;for(WALT5d971be8b8=(0x1e8b+418-0x202d);
...
(0x1528+3215-0x21b7);}else{WALT2a39967a03=WALTc0f271aeff[WALT5d971be8b8]>=
...
NUM_BUSY_BUCKETS-(0xc00+5955-0x2342));if(!WALT57acf293db)WALT57acf293db++;return
...
(0x2368+410-0x2502);int WALTc4b85e3bbe=NUM_BUSY_BUCKETS,final;u32 WALT083920bcc8
...
final<(0x54b+8287-0x25a8)){WALTf78d322c50=(0xe73+144-0xf03);final=
(0x99c+7237-0x25e0);}else{WALTf78d322c50=mult_frac(final,max_task_load(),
NUM_BUSY_BUCKETS);}WALT51a9118714=mult_frac(final+(0x1060+5675-0x268a),
max_task_load(),NUM_BUSY_BUCKETS);for(WALT5d971be8b8=(0x7b4+3482-0x154e);
...
WALT083920bcc8=(WALTf78d322c50+WALT51a9118714)/(0x14b7+4361-0x25be);
...
trace_sched_update_pred_demand(WALT0609dbaf9d,WALT8a690d3852,mult_frac((unsigned
 int)WALT75472ab857,(0x172d+91-0x1724),sched_ravg_window),WALT083920bcc8);return
...
unsigned long*WALT75c312d1cd){memset(WALT75c312d1cd,(0x4e2+266-0x5ec),
WALT64333e9737);__set_bit(NUM_LOAD_INDICES,WALT75c312d1cd);}static inline void 
clear_top_tasks_table(u8*table){memset(table,(0x17c7+3219-0x245a),
...
WALTdd3d5a87d4){u8 curr=rq->wrq.curr_table;u8 prev=(0x121+975-0x4ef)-curr;u8*
...
WALTbbf323d019]-=(0x7d7+271-0x8e5);if(curr_window)curr_table[WALT6a0597304c]+=
(0xa31+3215-0x16bf);if(WALT6a0597304c>rq->wrq.curr_top)rq->wrq.curr_top=
WALT6a0597304c;}if(!curr_table[WALTbbf323d019])__clear_bit(NUM_LOAD_INDICES-
WALTbbf323d019-(0x328+707-0x5ea),rq->wrq.top_tasks_bitmap[curr]);if(curr_table[
WALT6a0597304c]==(0x173+1545-0x77b))__set_bit(NUM_LOAD_INDICES-WALT6a0597304c-
(0xf92+1155-0x1414),rq->wrq.top_tasks_bitmap[curr]);return;}WALT64adf5cf6f=
WALTb7bb6f3305(prev_window);if(WALTdd3d5a87d4){if(prev_window){WALT78d0a88386[
WALT64adf5cf6f]+=(0x20a2+1389-0x260e);rq->wrq.prev_top=WALT64adf5cf6f;}if(
WALT78d0a88386[WALT64adf5cf6f]==(0x1977+2111-0x21b5))__set_bit(NUM_LOAD_INDICES-
WALT64adf5cf6f-(0x1e6+2592-0xc05),rq->wrq.top_tasks_bitmap[prev]);}else{
...
(0x8b8+1598-0xef5);WALT78d0a88386[WALT64adf5cf6f]+=(0xa8a+2742-0x153f);if(
...
(0x1f2b+779-0x2235),rq->wrq.top_tasks_bitmap[prev]);if(WALT78d0a88386[
WALT64adf5cf6f]==(0x898+2549-0x128c))__set_bit(NUM_LOAD_INDICES-WALT64adf5cf6f-
(0x736+2087-0xf5c),rq->wrq.top_tasks_bitmap[prev]);}}if(curr_window){curr_table[
WALT6a0597304c]+=(0x1738+795-0x1a52);if(WALT6a0597304c>rq->wrq.curr_top)rq->wrq.
curr_top=WALT6a0597304c;if(curr_table[WALT6a0597304c]==(0x1d+1745-0x6ed))
__set_bit(NUM_LOAD_INDICES-WALT6a0597304c-(0x1a2f+749-0x1d1b),rq->wrq.
...
(0x1331+1644-0x199c)-curr_table;int curr_top=rq->wrq.curr_top;
...
(0x1a32+2225-0x22e3);clear_top_tasks_table(rq->wrq.top_tasks[curr_table]);
WALT66971bc3d0(rq->wrq.top_tasks_bitmap[curr_table]);}rq->wrq.curr_table=
WALT78d0a88386;rq->wrq.prev_top=curr_top;rq->wrq.curr_top=(0xa28+6041-0x21c1);}
...
curr_window;int WALT5d971be8b8;curr_window=(0x1c1d+193-0x1cde);if(!
...
;WALT0609dbaf9d->wts.curr_window=(0x544+3018-0x110e);for(WALT5d971be8b8=
(0x549+1064-0x971);WALT5d971be8b8<nr_cpu_ids;WALT5d971be8b8++){WALT0609dbaf9d->
wts.prev_window_cpu[WALT5d971be8b8]=WALT1d6a0b8b72[WALT5d971be8b8];
WALT0609dbaf9d->wts.curr_window_cpu[WALT5d971be8b8]=(0x1ca4+2147-0x2507);}if(
...
WALTdedb054041)return(0x1a6+6607-0x1b75);return atomic_read(&rq->nr_iowait);}
...
WALT3205babe24==PICK_NEXT_TASK)return(0x1639+1304-0x1b51);return WALT59018c6914
||WALT40d96c83d6(rq);}if(WALT3205babe24==TASK_WAKE)return(0x9ca+3607-0x17e1);if(
WALT3205babe24==PUT_PREV_TASK||WALT3205babe24==IRQ_UPDATE)return
(0xdc9+4127-0x1de7);if(WALT3205babe24==TASK_UPDATE){if(rq->curr==WALT0609dbaf9d)
return(0x6a4+4006-0x1649);return WALT0609dbaf9d->on_rq?WALT88b9b0b9ce:
(0x478+7701-0x228d);}return WALT88b9b0b9ce;}
#define WALT20b422fc7b(WALT98927a0ef6, WALTc096799050) div64_u64((WALT98927a0ef6\
) + (WALTc096799050 - (0xfa0+2564-0x19a3)), WALTc096799050)
static inline u64 WALTb95ca9f31b(u64 WALT17abc14e87,struct rq*rq){return(
WALT17abc14e87*rq->wrq.task_exec_scale)>>(0x23bb+600-0x2609);}static inline 
...
){WALT36b7a61385=(0x6bb+2497-0x107c);WALT41bd9431ad=(0x954+896-0xcd4);
WALTc62485a0c5=(0xbdd+4602-0x1dd7);WALT1ae796a363=(0xbe6+2416-0x1556);}rq->wrq.
...
(0x192a+3167-0x2589);rq->wrq.nt_curr_runnable_sum=(0x1a8b+1798-0x2191);rq->wrq.
grp_time.curr_runnable_sum=(0x21bc+462-0x238a);rq->wrq.grp_time.
nt_curr_runnable_sum=(0xd53+3288-0x1a2b);}static void WALT130dee47f1(struct 
task_struct*WALT0609dbaf9d,struct rq*rq,int WALT3205babe24,u64 WALT790680e4aa,
u64 WALT59018c6914){int WALT3b5207d436,WALTdd3d5a87d4=(0x2d9+6413-0x1be6);int 
...
sched_predl)return(0xa95+2885-0x15da);WALT57acf293db=WALT5161a2cdf5(
...
(0x17e2+1881-0x1f3b);if(WALT3205babe24==TASK_WAKE||(!WALT19b86f3ee0&&(
WALT3205babe24==PICK_NEXT_TASK||WALT3205babe24==TASK_MIGRATE)))return
(0xc47+4564-0x1e1b);if(WALT3205babe24==PICK_NEXT_TASK&&rq->curr==rq->idle)return
(0x719+1787-0xe14);if(WALT3205babe24==TASK_UPDATE){if(rq->curr==WALT0609dbaf9d)
return(0xe5b+2460-0x17f6);return WALT0609dbaf9d->on_rq?WALT19b86f3ee0:
(0x2173+899-0x24f6);}return(0x53b+6306-0x1ddc);}unsigned int 
...
(0x749+7628-0x2515)];int WALT7a1e3fe84e,WALTc98feb876b;u32 max=
(0x1087+4745-0x2310),WALTf034c4ad57,demand,pred_demand;u64 sum=
(0x1d23+236-0x1e0f);u16 demand_scaled,pred_demand_scaled;if(!WALT8a690d3852||
is_idle_task(WALT0609dbaf9d)||!WALT7d946df0fb)goto WALT02a8afcd23;WALTc98feb876b
=WALTa591b9e54c-(0xa85+1350-0xfca);WALT7a1e3fe84e=WALTc98feb876b-WALT7d946df0fb;
for(;WALT7a1e3fe84e>=(0x64d+5317-0x1b12);--WALTc98feb876b,--WALT7a1e3fe84e){
...
WALT53b45fa17b[WALTc98feb876b];}for(WALTc98feb876b=(0x134+738-0x416);
...
WALT0609dbaf9d->wts.sum=(0xd44+425-0xeed);if(sysctl_sched_window_stats_policy==
...
WALT0609dbaf9d->wts.unfilter=max_t(int,(0x824+7321-0x24bd),WALT0609dbaf9d->wts.
...
WALT5807425a04(rq,WALT0609dbaf9d,WALT0609dbaf9d->wts.sum,(0x1587+4226-0x2608),
WALT3205babe24);return(0x4b3+8134-0x2479);}if(!WALT3b5207d436){return 
...
WALT0609dbaf9d,WALT0609dbaf9d->wts.sum,(0xb69+1501-0x1145),WALT3205babe24);if(
...
(0x1b2d+1448-0x20d5));rq->wrq.task_exec_scale=WALT20b422fc7b(WALT88024db1de*
...
task_struct*WALT0609dbaf9d,int init_load_pct){if(init_load_pct<(0x3cb+372-0x53f)
||init_load_pct>(0x1264+348-0x135c))return-EINVAL;WALT0609dbaf9d->wts.
init_load_pct=init_load_pct;return(0x1804+360-0x196c);}void init_new_task_load(
...
=current->wts.init_load_pct;WALT0609dbaf9d->wts.init_load_pct=(0x1f2+1341-0x72f)
;rcu_assign_pointer(WALT0609dbaf9d->wts.grp,NULL);INIT_LIST_HEAD(&WALT0609dbaf9d
->wts.grp_list);WALT0609dbaf9d->wts.mark_start=(0x471+2638-0xebf);WALT0609dbaf9d
->wts.sum=(0x9a+8315-0x2115);WALT0609dbaf9d->wts.curr_window=
(0x18f7+3411-0x264a);WALT0609dbaf9d->wts.prev_window=(0xf38+1483-0x1503);
WALT0609dbaf9d->wts.active_time=(0xbef+1942-0x1385);WALT0609dbaf9d->wts.
last_win_size=(0xad9+4848-0x1dc9);for(WALT5d971be8b8=(0xcb0+354-0xe12);
WALT5d971be8b8<NUM_BUSY_BUCKETS;++WALT5d971be8b8)WALT0609dbaf9d->wts.
busy_buckets[WALT5d971be8b8]=(0x395+1228-0x861);WALT0609dbaf9d->wts.cpu_cycles=
(0x934+1055-0xd53);WALT0609dbaf9d->wts.curr_window_cpu=kcalloc(nr_cpu_ids,sizeof
(u32),GFP_KERNEL|__GFP_NOFAIL);WALT0609dbaf9d->wts.prev_window_cpu=kcalloc(
nr_cpu_ids,sizeof(u32),GFP_KERNEL|__GFP_NOFAIL);if(init_load_pct){WALTb5ef75b5b9
=div64_u64((u64)init_load_pct*(u64)sched_ravg_window,(0x1176+4164-0x2156));
WALTbec959d3fa=WALTecf40170d4(WALTb5ef75b5b9);}WALT0609dbaf9d->wts.demand=
WALTb5ef75b5b9;WALT0609dbaf9d->wts.demand_scaled=WALTbec959d3fa;WALT0609dbaf9d->
wts.coloc_demand=WALTb5ef75b5b9;WALT0609dbaf9d->wts.pred_demand=
(0x1e9+6473-0x1b32);WALT0609dbaf9d->wts.pred_demand_scaled=(0x1a00+2568-0x2408);
for(WALT5d971be8b8=(0x22d+6420-0x1b41);WALT5d971be8b8<RAVG_HIST_SIZE_MAX;++
WALT5d971be8b8)WALT0609dbaf9d->wts.sum_history[WALT5d971be8b8]=WALTb5ef75b5b9;
WALT0609dbaf9d->wts.misfit=false;WALT0609dbaf9d->wts.rtg_high_prio=false;
...
WALT0609dbaf9d,(0xd73+3107-0x1996));free_task_load_ptrs(WALT0609dbaf9d);}void 
reset_task_stats(struct task_struct*WALT0609dbaf9d){int WALT5d971be8b8=
(0xebc+1465-0x1475);u32*WALTd74c5f0f49;u32*WALT72caf72ca7;WALTd74c5f0f49=
WALT0609dbaf9d->wts.curr_window_cpu;WALT72caf72ca7=WALT0609dbaf9d->wts.
prev_window_cpu;memset(WALTd74c5f0f49,(0xba6+4441-0x1cff),sizeof(u32)*nr_cpu_ids
);memset(WALT72caf72ca7,(0x7f2+2769-0x12c3),sizeof(u32)*nr_cpu_ids);
WALT0609dbaf9d->wts.mark_start=(0xd33+976-0x1103);WALT0609dbaf9d->wts.sum=
(0x4cb+8286-0x2529);WALT0609dbaf9d->wts.demand=(0x1367+4389-0x248c);
WALT0609dbaf9d->wts.coloc_demand=(0x1061+4001-0x2002);for(WALT5d971be8b8=
(0x462+6507-0x1dcd);WALT5d971be8b8<RAVG_HIST_SIZE_MAX;++WALT5d971be8b8)
WALT0609dbaf9d->wts.sum_history[WALT5d971be8b8]=(0x2f9+2419-0xc6c);
WALT0609dbaf9d->wts.curr_window=(0x1675+2517-0x204a);WALT0609dbaf9d->wts.
prev_window=(0x1659+2684-0x20d5);WALT0609dbaf9d->wts.pred_demand=
(0x18b8+2573-0x22c5);for(WALT5d971be8b8=(0xb2d+6556-0x24c9);WALT5d971be8b8<
NUM_BUSY_BUCKETS;++WALT5d971be8b8)WALT0609dbaf9d->wts.busy_buckets[
WALT5d971be8b8]=(0x67a+8007-0x25c1);WALT0609dbaf9d->wts.demand_scaled=
(0x112a+5464-0x2682);WALT0609dbaf9d->wts.pred_demand_scaled=(0x142f+812-0x175b);
WALT0609dbaf9d->wts.active_time=(0xcb5+6532-0x2639);WALT0609dbaf9d->wts.
last_win_size=(0x1294+2890-0x1dde);WALT0609dbaf9d->wts.curr_window_cpu=
...
unsigned int __read_mostly sysctl_sched_group_upmigrate_pct=(0xbe2+3904-0x1abe);
unsigned int __read_mostly sched_group_downmigrate=19000000;unsigned int 
__read_mostly sysctl_sched_group_downmigrate_pct=(0x56d+3666-0x1360);static 
inline void WALTfbcbf0e27a(void){unsigned int WALT6ace768b42=
arch_scale_cpu_capacity(cluster_first_cpu(sched_cluster[(0xb13+2331-0x142e)]));
...
(0x783+797-0xa3c));sched_group_downmigrate=div64_ul(WALT205fc9f073*
sysctl_sched_group_downmigrate_pct,(0x621+4649-0x17e6));}struct 
...
WALT2d02e5bf36.list),.id=(0xe4a+3892-0x1d7e),.cur_freq=(0x144a+3185-0x20ba),.
max_possible_freq=(0x56b+2167-0xde1),.aggr_grp_load=(0x10c6+2548-0x1aba),};void 
...
(0x1710+3168-0x236f);cluster->max_possible_freq=(0x92f+1530-0xf28);
...
walt_sched_cluster*cluster;int WALT7609b61ad1=(0x12a2+362-0x140c);
...
WALTa85cf99964=(0xc68+3843-0x1b6b),WALTd350724547=U64_MAX;unsigned long flags;
...
;for(WALT5d971be8b8=(0x1495+56-0x14cd);WALT5d971be8b8<num_sched_clusters;
...
WALT5d971be8b8=(0x8c7+5255-0x1d4e);WALT5d971be8b8<num_sched_clusters;
WALT5d971be8b8++){int WALT2dc758aa44,WALT041cb91678=(0x11e9+2459-0x1b83);
cpumask_copy(&WALTe421f3d6a7[WALT5d971be8b8][(0x314+2986-0xebe)],&sched_cluster[
WALT5d971be8b8]->cpus);for(WALT2dc758aa44=WALT5d971be8b8+(0x1be0+2486-0x2595);
...
WALT041cb91678++;}for(WALT2dc758aa44=WALT5d971be8b8-(0x65d+3095-0x1273);
WALT2dc758aa44>=(0xba0+3250-0x1852);WALT2dc758aa44--){cpumask_copy(&
...
WALTf48c6e4ef8)){WARN((0xf58+5477-0x24bc),
...
cluster){if(cpumask_weight(&cluster->cpus)==(0x538+3888-0x1467))cpumask_or(&
asym_cap_sibling_cpus,&asym_cap_sibling_cpus,&cluster->cpus);}if(cpumask_weight(
&asym_cap_sibling_cpus)==(0x10c0+2389-0x1a14))cpumask_clear(&
asym_cap_sibling_cpus);WALTb9b0dca770=WALT516ffeae8c();if(!WALTb9b0dca770){
BUG_ON((0x765+3443-0x14d7));return;}smp_store_release(&cpu_array,WALTb9b0dca770)
...
TASK_UPDATE,sched_ktime_clock(),(0x1999+873-0x1d02));raw_spin_unlock_irqrestore(
...
CPUFREQ_TRANSITION_NOTIFIER);return(0xbd5+2731-0x1680);}static void 
...
sysctl_sched_coloc_downmigrate_ns;struct walt_related_thread_group*
WALT7c7bf2c77f[MAX_NUM_CGROUP_COLOC_ID];static LIST_HEAD(WALT929dec1bfc);static 
DEFINE_RWLOCK(WALTc37082022e);static inline void WALTb7252e9a3d(struct 
walt_related_thread_group*grp,u64 demand,bool WALTf4f061ddc1){if(WALTf4f061ddc1)
{grp->skip_min=false;return;}if(is_suh_max())demand=sched_group_upmigrate;if(!
grp->skip_min){if(demand>=sched_group_upmigrate){grp->skip_min=true;}return;}if(
demand<sched_group_downmigrate){if(!sysctl_sched_coloc_downmigrate_ns){grp->
skip_min=false;return;}if(!grp->downmigrate_ts){grp->downmigrate_ts=grp->
last_update;return;}if(grp->last_update-grp->downmigrate_ts>
sysctl_sched_coloc_downmigrate_ns){grp->downmigrate_ts=(0xed0+5415-0x23f7);grp->
skip_min=false;}}else if(grp->downmigrate_ts)grp->downmigrate_ts=
(0xe10+2679-0x1887);}int preferred_cluster(struct walt_sched_cluster*cluster,
struct task_struct*WALT0609dbaf9d){struct walt_related_thread_group*grp;int 
WALT094475d445=-(0x871+4928-0x1bb0);rcu_read_lock();grp=
...
WALTb53071212f(struct walt_related_thread_group*grp){struct task_struct*
WALT0609dbaf9d;u64 WALTd26a455a6d=(0x1cf+2137-0xa28);bool WALT509eaf33de=false;
...
last_update<sched_ravg_window/(0x1195+2297-0x1a84))return;list_for_each_entry(
WALT0609dbaf9d,&grp->tasks,wts.grp_list){if(task_boost_policy(WALT0609dbaf9d)==
SCHED_BOOST_ON_BIG){WALT509eaf33de=true;break;}if(WALT0609dbaf9d->wts.mark_start
<WALT790680e4aa-(sched_ravg_window*WALTa591b9e54c))continue;WALTd26a455a6d+=
WALT0609dbaf9d->wts.coloc_demand;if(!trace_sched_set_preferred_cluster_enabled()
){if(WALTd26a455a6d>sched_group_upmigrate)break;}}grp->last_update=
WALT790680e4aa;WALTb7252e9a3d(grp,WALTd26a455a6d,WALT509eaf33de);
trace_sched_set_preferred_cluster(grp,WALTd26a455a6d);WALTe1f307999b:if(grp->id
==DEFAULT_CGROUP_COLOC_ID&&grp->skip_min!=WALT4288c41a52){if(grp->skip_min)grp->
start_ts=sched_clock();sched_update_hyst_times();}}void set_preferred_cluster(
struct walt_related_thread_group*grp){raw_spin_lock(&grp->lock);WALTb53071212f(
grp);raw_spin_unlock(&grp->lock);}int update_preferred_cluster(struct 
...
;if(!grp)return(0x1d8b+547-0x1fae);if(unlikely(WALT53c3aaf16d&&is_suh_max()))
return(0xcdf+3589-0x1ae3);if(abs(WALT30f2685d2b-WALT3678748cf3)>
sched_ravg_window/(0x9e9+187-0xaa0)||sched_ktime_clock()-grp->last_update>
sched_ravg_window)return(0x13e7+2011-0x1bc1);return(0x41c+8390-0x24e2);}
#define WALTdebf0e8b8a	(0x6e2+1448-0xc8a)
#define WALT56968f5b11	(0x9b4+85-0xa08)
...
walt_related_thread_group*grp;for(WALT5d971be8b8=(0x3ff+4244-0x1492);
...
(0x617+8293-0x267c);WALT2839a901be:for(WALT5d971be8b8=(0x9c3+6930-0x24d4);
...
struct rq*rq;int WALTebef427696=(0xd28+4157-0x1d64);struct rq_flags 
...
list_empty(&grp->tasks)){WALTebef427696=(0x29c+1626-0x8f6);WALTb53071212f(grp);}
...
(0x7f+2303-0x97e);unsigned long flags;struct walt_related_thread_group*grp=NULL;
...
WALT0609dbaf9d);WALTe3d424fbb1=grp?grp->id:(0xaa9+983-0xe80);rcu_read_unlock();
...
write_unlock_irqrestore(&WALTc37082022e,flags);return(0x473+4136-0x149b);}
...
DEFAULT_CGROUP_COLOC_ID:(0x155a+3463-0x22e1);return WALT3f0a317889(
...
,rq,TASK_UPDATE,WALT790680e4aa,(0x4+5467-0x155f));walt_update_task_ravg(
WALT0609dbaf9d,rq,TASK_UPDATE,WALT790680e4aa,(0xd9f+1471-0x135e));WALT5eebbbffe9
...
(0x437+8070-0x23bc));}*WALT42b4427a2c-=WALT0609dbaf9d->wts.curr_window_cpu[cpu];
...
(0x731+1884-0xe8c));}*WALT262b512e81-=WALT0609dbaf9d->wts.prev_window_cpu[cpu];
...
curr_window_cpu[cpu]);WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac(
(0xb65+5777-0x21f5));}*WALTf7e69c10fd-=WALT0609dbaf9d->wts.curr_window_cpu[cpu];
...
prev_window_cpu[cpu]);WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac(
(0x9c2+1458-0xf73));}*WALT7ba10de4f4-=WALT0609dbaf9d->wts.prev_window_cpu[cpu];}
...
,WALT0609dbaf9d->pid,cpu,WALT3205babe24,*WALT42b4427a2c,WALT0609dbaf9d->wts.
curr_window);WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac((0xd60+3514-0x1b19));
}*WALT42b4427a2c-=WALT0609dbaf9d->wts.curr_window;if(*WALT262b512e81<
WALT0609dbaf9d->wts.prev_window){printk_deferred(
""
,WALT0609dbaf9d->pid,cpu,WALT3205babe24,*WALT262b512e81,WALT0609dbaf9d->wts.
prev_window);WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac((0x62+6940-0x1b7d));}
...
""
,WALT0609dbaf9d->pid,cpu,WALT3205babe24,*WALTf7e69c10fd,WALT0609dbaf9d->wts.
curr_window);WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac((0xe7d+1579-0x14a7));
}*WALTf7e69c10fd-=WALT0609dbaf9d->wts.curr_window;if(*WALT7ba10de4f4<
WALT0609dbaf9d->wts.prev_window){printk_deferred(
""
,WALT0609dbaf9d->pid,cpu,WALT3205babe24,*WALT7ba10de4f4,WALT0609dbaf9d->wts.
prev_window);WALT1efe882752(WALT0609dbaf9d);WALT609fba09ac((0x6d8+6492-0x2033));
...
(0xce1+4915-0x2014);WALT0609dbaf9d->wts.prev_window_cpu[WALT5d971be8b8]=
(0x141a+442-0x15d4);}}*WALT2e4da9dbb4+=WALT0609dbaf9d->wts.curr_window;*
WALT086b45bedf+=WALT0609dbaf9d->wts.prev_window;if(WALT5eebbbffe9){*
...
(0x561+8185-0x255a);}static void WALT82ef581fb8(void);static void WALTaea65e35d7
(void){if(likely(num_sched_clusters>(0x7c7+7052-0x2353)))WALTfbcbf0e27a();
WALT82ef581fb8();}static void WALT7901ba6885(struct rq*rq){u64 WALTa820aa7542=
(0xa0f+531-0xc22);unsigned int WALT09570395b5=(0x129+565-0x35e);u64 
...
last_irq_window,sched_ravg_window);if(WALT09570395b5<(0x1317+2239-0x1bcc))rq->
wrq.avg_irqload=mult_frac(rq->wrq.avg_irqload,(0x4df+4574-0x16ba),
(0x724+1667-0xda3));else rq->wrq.avg_irqload=(0xea9+1442-0x144b);WALT597a17678f=
irq_time_read(cpu_of(rq));if(WALT597a17678f>rq->wrq.prev_irq_time)WALTa820aa7542
=WALT597a17678f-rq->wrq.prev_irq_time;rq->wrq.avg_irqload+=WALTa820aa7542;rq->
wrq.prev_irq_time=WALT597a17678f;if(WALT09570395b5<SCHED_HIGH_IRQ_TIMEOUT)rq->
wrq.high_irqload=(rq->wrq.avg_irqload>=WALTfb43f308cb);else rq->wrq.high_irqload
=(0x12e7+4052-0x22bb);}void walt_irq_work(struct irq_work*irq_work){struct 
walt_sched_cluster*cluster;struct rq*rq;int cpu;u64 WALT0cc0a342b9;bool 
WALTd7ac89c463=false,WALTfc516a8062=false;u64 WALT392940921f=(0xcaf+5253-0x2134)
,WALT9ef535cb5f=(0xdf4+5077-0x21c9);int WALT8025f196c5=(0x1387+469-0x155c);u64 
WALTaeec67ca96;unsigned long flags;if(irq_work==&WALT72f8d06752)WALTd7ac89c463=
true;for_each_cpu(cpu,cpu_possible_mask){if(WALT8025f196c5==(0x1025+5014-0x23bb)
...
(0xa3d+5544-0x1fe5);raw_spin_lock(&cluster->load_lock);for_each_cpu(cpu,&cluster
->cpus){rq=cpu_rq(cpu);if(rq->curr){walt_update_task_ravg(rq->curr,rq,
TASK_UPDATE,WALT0cc0a342b9,(0x1924+1988-0x20e8));WALTa647ab699e(rq);
aggr_grp_load+=rq->wrq.grp_time.prev_runnable_sum;}if(WALTd7ac89c463&&rq->wrq.
notif_pending&&cpumask_test_cpu(cpu,&asym_cap_sibling_cpus)){WALTfc516a8062=true
;rq->wrq.notif_pending=false;}}cluster->aggr_grp_load=aggr_grp_load;
WALT392940921f+=aggr_grp_load;if(is_min_capacity_cluster(cluster))WALT9ef535cb5f
=aggr_grp_load;raw_spin_unlock(&cluster->load_lock);}if(WALT392940921f){if(
cpumask_weight(&asym_cap_sibling_cpus)){u64 WALT2a9650c76f=WALT392940921f-
WALT9ef535cb5f;for_each_cpu(cpu,&asym_cap_sibling_cpus)cpu_cluster(cpu)->
aggr_grp_load=WALT2a9650c76f;}rtgb_active=is_rtgb_active();}else{rtgb_active=
false;}if(!WALTd7ac89c463&&sysctl_sched_user_hint&&time_after(jiffies,
WALTfac0edc4a3))sysctl_sched_user_hint=(0x242+560-0x472);for_each_sched_cluster(
cluster){cpumask_t WALT8fc9e418fd;unsigned int WALT3fc386a32e,WALT5d971be8b8=
(0x2f3+5883-0x19ed);cpumask_and(&WALT8fc9e418fd,&cluster->cpus,cpu_online_mask);
...
walt_rotation_enabled=(0x13f4+1449-0x199d);return;}walt_rotation_enabled=
...
flags;u64 WALTd8ca2b9a3c=(0xc67+771-0xf6a),WALT790680e4aa;struct task_struct*
WALT0609dbaf9d;int WALTba0d35d300,WALT4e46740d20=(0x14e7+1284-0x15eb);struct 
walt_sched_cluster*cluster;int WALT5d971be8b8=(0xe62+614-0x10c8);grp=
...
(0x1610+2529-0x1ff0))WALT4e46740d20=arch_scale_cpu_capacity(WALTba0d35d300);data
->coloc_load_pct=div64_u64(WALTd8ca2b9a3c*(0xc37+7464-0x255f)*(0xf27+972-0x128f)
...
ta_util_pct[WALT5d971be8b8]=div64_u64(cluster->aggr_grp_load*(0xae2+6104-0x1eba)
*(0x17d2+1472-0x1d2e),(u64)sched_ravg_window*WALT4e46740d20);WALT4e46740d20=
arch_scale_freq_capacity(WALT61c3a9ab4e);data->cur_cap_pct[WALT5d971be8b8]=(
WALT4e46740d20*(0x1d79+148-0x1da9))/(0x40d+1740-0x6d9);WALT5d971be8b8++;}}int 
...
num_sched_clusters<=(0x6d6+44-0x702)))return-EPERM;mutex_lock(&WALT8a2c4b3219);
...
(0x1ae1+187-0x1b38);WALTd07944bdc3=sched_ravg_window>>SCHED_CAPACITY_SHIFT;
sched_init_task_load_windows=div64_u64((u64)WALT8bf7fc10da*(u64)
sched_ravg_window,(0xf0c+5056-0x2268));WALTb0bc76dcf5=WALTecf40170d4(
sched_init_task_load_windows);WALTfb43f308cb=div64_u64((u64)sched_ravg_window*
(0x131b+1801-0x19c5),(u64)(0x1c2f+362-0x1d35));}static void WALT42949ab86e(void)
...
(struct rq*rq){int WALT2dc758aa44;if(cpu_of(rq)==(0x8d0+2783-0x13af))
WALT42949ab86e();cpumask_set_cpu(cpu_of(rq),&rq->wrq.freq_domain_cpumask);rq->
wrq.walt_stats.cumulative_runnable_avg_scaled=(0x1b66+1779-0x2259);rq->wrq.
prev_window_size=sched_ravg_window;rq->wrq.window_start=(0x0+7425-0x1d01);rq->
wrq.walt_stats.nr_big_tasks=(0x14db+1910-0x1c51);rq->wrq.walt_flags=
(0x1db0+1232-0x2280);rq->wrq.avg_irqload=(0x162c+2440-0x1fb4);rq->wrq.
prev_irq_time=(0x52a+668-0x7c6);rq->wrq.last_irq_window=(0x8ff+7623-0x26c6);rq->
wrq.high_irqload=false;rq->wrq.task_exec_scale=(0xced+921-0xc86);rq->wrq.
push_task=NULL;rq->wrq.cluster=&WALT2d02e5bf36;rq->wrq.curr_runnable_sum=rq->wrq
.prev_runnable_sum=(0x1bfd+862-0x1f5b);rq->wrq.nt_curr_runnable_sum=rq->wrq.
nt_prev_runnable_sum=(0x154b+2409-0x1eb4);memset(&rq->wrq.grp_time,
(0xc28+3544-0x1a00),sizeof(struct group_cpu_time));rq->wrq.old_busy_time=
(0x17da+3751-0x2681);rq->wrq.old_estimated_time=(0x147a+3729-0x230b);rq->wrq.
walt_stats.pred_demands_sum_scaled=(0xc7b+3632-0x1aab);rq->wrq.walt_stats.
nr_rtg_high_prio_tasks=(0xc77+5897-0x2380);rq->wrq.ed_task=NULL;rq->wrq.
curr_table=(0x17bb+3515-0x2576);rq->wrq.prev_top=(0x1786+2915-0x22e9);rq->wrq.
curr_top=(0x2e3+3259-0xf9e);rq->wrq.last_cc_update=(0x16f7+3502-0x24a5);rq->wrq.
cycles=(0x167+5933-0x1894);for(WALT2dc758aa44=(0x1c52+1351-0x2199);
WALT2dc758aa44<NUM_TRACKED_WINDOWS;WALT2dc758aa44++){memset(&rq->wrq.load_subs[
WALT2dc758aa44],(0xfa8+4568-0x2180),sizeof(struct load_subtractions));rq->wrq.
...
WALT2dc758aa44]);}rq->wrq.cum_window_demand_scaled=(0x37a+8435-0x246d);rq->wrq.
...
WALT8a2c4b3219);if(WALT123284dc15&&(HZ!=(0x1ac+2650-0xb0c)||!
